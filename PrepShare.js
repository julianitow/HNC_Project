...
function updateShareData() {
	var RunGetShare = setInterval(GetShareData,10000); // Sets the run interval to 10sec
}

function MoveShares() {
	var RunMoveShares = setInterval(MoveShareData,1500); // Sets the run interval to 5sec
}
		...
		if(XHR.readyState==4) {
			if(XHR.status==200) {
				...
				for(i=0;i < responsedata.length;i++){
					var Money = fx(responsedata[i].CurPrice).convert({ rom:"GBP", to: strUser });// Using fx Function get the share value in the selected currency
					var Change = fx(responsedata[i].Chg).convert({ rom:"GBP", to: strUser }); // Using fx Function get the cahnge valuse in the selected currency
					//console.log("Change: "+Change); // TESTing of change value
					...
					var options=document.createElement("p"); // Cretes a new paragraph element
						...
						options.innerHTML= "Trading ID: "+responsedata[i].Sym+"<br/>Name :"+responsedata[i].Name+"<br/>Current Price: "+strUser+" "+parseFloat(Money).toFixed(2)+"<br/>Change : "+parseFloat(Change).toFixed(2); // Added V1.01
		...
		if(XHR.readyState==4) {
			if(XHR.status==200) {
				...
				for(i=0;i < responsedata.length;i++){
					...
					var Change = fx(responsedata[i].Chg).convert({ rom:"GBP", to: strUser });
					...
					options.innerHTML= "Trading ID: "+responsedata[i].Sym+"<br/>Name :"+responsedata[i].Name+"<br/>Current Price: "+strUser+" "+parseFloat(Money).toFixed(2)+"<br/>Change : "+parseFloat(Change).toFixed(2);; // Updated V1.01
					newarticle.appendChild(options);
				}
				var d = new Date();
				document.getElementById("Updated").innerHTML = d;
...
function MoveShareData(){
	//Gets the second child element and move it behind the last child element.
	console.log("Running Share Move")
	var main=document.getElementById("FsteScroll");
	var ChildElm = main.childNodes[2];
	main.appendChild(ChildElm);
